...
public:
	...
	int FindIndexFromObject(CObject *pObjTarget)
	{
		if (pObjTarget != NULL)
		{
			POSITION pos = m_listControl.GetHeadPosition();
	...
	CObject *FindObjectFromIndex(int iIndexTarget)
	{
		if (iIndexTarget >= 0)
		{
			POSITION pos = m_listControl.FindIndex(iIndexTarget);
			CObject *pObj = NULL;
			if (pos)
				pObj = m_listControl.GetAt(pos);
			return pObj;
		}
		return NULL;
	}
